...
#         Perl_Charnames(cp)
# 
# where 'cp' is one of the 110_569 code points in Unicode Version 6.1.0 for
# which the mapping is not to the null string.
# 
...
# entries are used only for nameless code points in the Name property.  The
# Name_Alias property adds duplicate code point entries that are alternatives
# to the original name.  If an addition is a corrected name, it will be
# physically first in the table.  The original (less correct, but still
# valid) name will be next; then any alternatives, in no particular order;
# and finally any abbreviations, again in no particular order.
# 
# This file doesn't include the algorithmically determinable names.  For
# those, use 'unicore/Name.pm'.
# 
